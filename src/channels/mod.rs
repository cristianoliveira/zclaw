...
fn conversation_history_key(msg: &traits::ChannelMessage) -> String {
    // Include reply_target for conversation isolation. Channels that need
...
fn build_channel_by_id(config: &Config, channel_id: &str) -> Result<Arc<dyn Channel>> {
    match channel_id {
        "telegram" => {
            ...
            // Bot token: environment variable > config file
            let bot_token = std::env::var("ZEROCLAW_TELEGRAM_BOT_TOKEN")
                .ok()
                .and_then(|token| {
                    let token = token.trim();
                    (!token.is_empty()).then(|| token.to_owned())
                })
                .unwrap_or_else(|| tg.bot_token.clone());
            let ack = tg
                .ack_reactions
...
This is an example JSON object for profile settings."#;
    ...
    #[test]
    fn interruption_scope_key_without_scope_id_is_two_component() {
        let msg = traits::ChannelMessage {
            id: "1".into(),
            sender: "alice".into(),
        ...
        assert_eq!(interruption_scope_key(&msg), "matrix_room");
    }

    #[test]
    fn interruption_scope_key_with_scope_id_is_three_component() {
        let msg = traits::ChannelMessage {
            id: "1".into(),
            sender: "alice".into(),
            reply_target: "room".into(),
        ...
        assert_eq!(interruption_scope_key(&msg), "matrix_room_$thread1");
    }
    ...
    fn interruption_scope_key_thread_ts_alone_does_not_affect_key() {
        ...
        assert_eq!(interruption_scope_key(&msg), "slack_C123");
    }
    ...
    #[test]
    fn telegram_bot_token_env_var_overrides_config() {
        // Clean up any existing env var first
        std::env::remove_var("ZEROCLAW_TELEGRAM_BOT_TOKEN");

        // Set env var
        std::env::set_var("ZEROCLAW_TELEGRAM_BOT_TOKEN", "env-token-12345");

        // Create config with a different token
        let config_token = "config-token-67890".to_string();

        // Verify that when building the channel, env var takes precedence
        let bot_token = std::env::var("ZEROCLAW_TELEGRAM_BOT_TOKEN")
            .ok()
            .and_then(|token| {
                let token = token.trim();
            ...
            .unwrap_or_else(|| config_token.clone());

        assert_eq!(bot_token, "env-token-12345", "Environment variable should override config token");
    ...
    #[test]
    fn telegram_bot_token_fallback_to_config_when_env_not_set() {
        // Ensure env var is NOT set
        std::env::remove_var("ZEROCLAW_TELEGRAM_BOT_TOKEN");

        // Create config with a token
        let config_token = "config-token-abcdef";
        let bot_token = std::env::var("ZEROCLAW_TELEGRAM_BOT_TOKEN")
            .ok()
            .and_then(|token| {
                let token = token.trim();
        ...
        assert_eq!(bot_token, config_token, "Should fall back to config token when env var is not set");
    }

    #[test]
    fn telegram_bot_token_empty_env_var_fallback_to_config() {
        // Clean up first
        std::env::remove_var("ZEROCLAW_TELEGRAM_BOT_TOKEN");

        // Set env var to empty string
        std::env::set_var("ZEROCLAW_TELEGRAM_BOT_TOKEN", "");

        let config_token = "config-token-xyz";
        let bot_token = std::env::var("ZEROCLAW_TELEGRAM_BOT_TOKEN")
            .ok()
            .and_then(|token| {
                let token = token.trim();
        ...
        assert_eq!(bot_token, config_token, "Should fall back to config token when env var is empty");
